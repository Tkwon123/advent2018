...
readFile('../data.txt').then( output=>{
  ...
  let highestAsleepMin = 0
  let highestAsleepGuard = results.reduce((acc,cv)=>{
    let compareVal = Math.max(...Object.values(cv.minutes))
    if ( compareVal > highestAsleepMin) {
      highestAsleepMin = compareVal
      return cv
    } else {
      return acc
    }
  ...
  // multiply the highest min by the guard
  console.log(highestAsleepGuard)
})

function countAsleep(array){
  array.forEach( item=>{
    try {
      if (item.includes("#")) {
